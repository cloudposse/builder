...
if [ -n "${DOCKER_HOST}" ]; then
	print_msg "=> Detected external docker host"
	...
	docker version > /dev/null 2>&1 || { print_msg "   Failed to connect to docker daemon at $DOCKER_HOST" && exit 1; }
	EXTERNAL_DOCKER=yes
	DOCKER_USED="Using external docker ${DOCKER_VERSION} at $DOCKER_HOST"
